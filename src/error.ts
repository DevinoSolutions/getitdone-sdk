/**
 * Typed error hierarchy for the GetItDone public API (open-api D3/D9).
 *
 ...
 *
 * Types come from `@getitdone/api-contracts` (bundled at build time) — the
 * SAME zod schemas the server validates with, so there is no drift.
 */
import type { FieldError, Problem, ProblemCode } from '@getitdone/api-contracts'

/** Base class of every error this SDK throws. */
export class GetItDoneError extends Error {}

/** Request never produced an HTTP response (DNS/TLS/socket failure). */
export class APIConnectionError extends GetItDoneError {
...
export class APIUserAbortError extends GetItDoneError {
...
export class APIError extends GetItDoneError {
