 ...
 *
 * The verifier is re-exported from `@getitdone/api-contracts` — the EXACT
 * code the GetItDone delivery worker signs with (bundled at build time).
 * Secret GENERATION stays server-side in the product and is deliberately
 * not shipped here.
 */
export {
...
export function verifyWebhook(
    ...
    return verifyWebhookSignature({
        ...
        secrets:
            typeof input.secret === 'string' ? [input.secret] : input.secret,
