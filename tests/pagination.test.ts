...
const task = (id: string) => ({ id, title: `Task ${id}` })
...
describe('for-await auto-iteration across pages', () => {
    it('walks all three queued pages in order and re-sends the original limit alongside each after cursor', async () => {
        const harness = createQueuedFetch([
            listReply([task('T-1'), task('T-2')], 'cur_page2'),
            listReply([task('T-3'), task('T-4')], 'cur_page3'),
            listReply([task('T-5')], null),
        ])
        const client = makeTestClient(harness)

        const seenIds: string[] = []
        for await (const item of client.tasks.list({ limit: 2 })) {
            seenIds.push(item.id)
        }

        expect(seenIds).toEqual(['T-1', 'T-2', 'T-3', 'T-4', 'T-5'])
        ...
        expect(
            harness.calls.every(call =>
                call.url.startsWith(`${TEST_BASE_URL}/v1/tasks`),
            ),
        ).toBe(true)
    })
})
...
describe('awaited Page escape hatch', () => {
    it('exposes data, hasMore and nextCursor on the awaited page, fetches page two via getNextPage, and returns null from getNextPage on the last page', async () => {
        const harness = createQueuedFetch([
            listReply([task('T-1'), task('T-2')], 'cur_page2'),
            listReply([task('T-3')], null),
        ])
        const client = makeTestClient(harness)

        const page = await client.tasks.list({ limit: 2 })
        expect(page.data.map(item => item.id)).toEqual(['T-1', 'T-2'])
        expect(page.hasMore).toBe(true)
        expect(page.nextCursor).toBe('cur_page2')

        const nextPage = await page.getNextPage()
        expect(nextPage).not.toBeNull()
        expect(nextPage?.data.map(item => item.id)).toEqual(['T-3'])
    ...
    it('crosses into the remaining pages when for-await iterating a resolved Page instance directly (Symbol.asyncIterator on Page)', async () => {
        ...
        expect(seenIds).toEqual(['T-1', 'T-2', 'T-3'])
        expect(harness.calls).toHaveLength(2)
    })
})

describe('single-page lists', () => {
    it('performs exactly one fetch when an org-scoped members list answers has_more false and next_cursor null', async () => {
        const harness = createQueuedFetch([
            listReply(
                [
                    { id: 'user_1', name: 'Ada' },
        ...
        for await (const member of client.members.list()) {
            seenIds.push(member.id)
        }

        expect(seenIds).toEqual(['user_1', 'user_2'])
        expect(harness.calls).toHaveLength(1)
        expect(harness.calls[0]?.url).toBe(`${TEST_BASE_URL}/v1/members`)
    })
})
