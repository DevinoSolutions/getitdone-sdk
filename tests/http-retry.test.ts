...
describe('HTTP 5xx retries on safe requests', () => {
    it('retries a GET that answers 500 once and resolves with the second (200) response body', async () => {
        const harness = createQueuedFetch([
            ...
            {
                status: 200,
                jsonBody: { id: 'T-1', title: 'Recovered after retry' },
                headers: { 'x-request-id': 'req_recovered' },
            },
        ])
        const client = makeTestClient(harness)
        const task = await client.tasks.retrieve('T-1')
    ...
    it('throws InternalServerError after maxRetries+1 total fetch calls when a GET answers 500 on every attempt (default budget 2 retries → 3 calls)', async () => {
        ...
        const client = makeTestClient(harness)
        await expect(client.tasks.retrieve('T-1')).rejects.toBeInstanceOf(
            InternalServerError,
        )
...
describe('429 rate limiting and the Retry-After ceiling', () => {
    it('retries a 429 rate_limited response whose Retry-After is 0 and succeeds on the second attempt', async () => {
        const harness = createQueuedFetch([
            ...
            {
                status: 200,
                jsonBody: { id: 'T-2', title: 'Made it past the burst window' },
            },
        ])
        const client = makeTestClient(harness)
        const task = await client.tasks.retrieve('T-2')
    ...
    it('reports isQuotaExhausted true on a RateLimitError carrying the quota_exhausted problem code', async () => {
        const harness = createQueuedFetch([
            problemReply(429, 'quota_exhausted'),
        ])
        const client = makeTestClient(harness, { maxRetries: 0 })
        const error = await client.tasks
            .retrieve('T-4')
...
describe('POST retry safety and idempotency keys', () => {
    it('never retries a 500 on a POST sent without any idempotency key (the non-idempotent verify probe performs exactly one call)', async () => {
        const harness = createQueuedFetch([retryableInternalError()])
        const client = makeTestClient(harness)
        await expect(
            client.request({
                method: 'POST',
                path: '/v1/webhook-endpoints/x/verify',
            }),
    ...
    it('auto-generates a gid-sdk- prefixed idempotency key for an idempotent-declared POST and replays the SAME key on the retry after a 500', async () => {
        const harness = createQueuedFetch([
            retryableInternalError(),
            { status: 201, jsonBody: { id: 'T-9', title: 'Created once' } },
        ])
        const client = makeTestClient(harness)
        const task = await client.tasks.create({ title: 'Created once' })
        expect(task.id).toBe('T-9')
        expect(harness.calls).toHaveLength(2)
        const firstKey = harness.calls[0]?.headers['idempotency-key']
        expect(firstKey).toMatch(/^gid-sdk-/)
    ...
    it('sends an explicit options.idempotencyKey verbatim instead of auto-generating one', async () => {
        const harness = createQueuedFetch([
            { status: 201, jsonBody: { id: 'T-10', title: 'Explicit key' } },
        ])
        const client = makeTestClient(harness)
    ...
    it('sends no idempotency-key header at all on GET requests', async () => {
        const harness = createQueuedFetch([
            { status: 200, jsonBody: { id: 'T-11', title: 'Read-only' } },
        ])
        const client = makeTestClient(harness)
        await client.tasks.retrieve('T-11')
    ...
    it('retries a 409 received while an idempotency key was sent (idempotency_in_progress resolves to a replay)', async () => {
        const harness = createQueuedFetch([
            ...
            { status: 201, jsonBody: { id: 'T-12', title: 'Replayed' } },
        ])
        const client = makeTestClient(harness)
        const task = await client.tasks.create({ title: 'Replayed' })
        expect(task.id).toBe('T-12')
        expect(harness.calls).toHaveLength(2)
    })
    ...
    it('does not retry a 409 on a request that carried no idempotency key (PATCH) and throws ConflictError after one call', async () => {
        ...
        const client = makeTestClient(harness)
        await expect(
            client.tasks.update('T-13', { title: 'Conflicted' }),
...
describe('caller aborts and per-attempt timeouts', () => {
    it('rejects with APIUserAbortError before attempting any fetch when the caller signal is already aborted', async () => {
        ...
        controller.abort()
        await expect(
            client.tasks.retrieve('T-14', { signal: controller.signal }),
    ...
    it('rejects with APIUserAbortError and performs no retry when the caller signal fires mid-flight', async () => {
        ...
        const client = makeTestClient(harness)
        const controller = new AbortController()
        // The SDK invokes fetch synchronously on call, so aborting right
        // after starting the request is a genuine mid-flight abort.
        const pending = client.tasks.retrieve('T-15', {
    ...
    it('rejects with APIConnectionTimeoutError when the per-attempt timeout signal fires on a fetch that never resolves (timeoutMs 5, maxRetries 0)', async () => {
        ...
        const client = makeTestClient(harness, { maxRetries: 0 })
        await expect(
            client.tasks.retrieve('T-16', { timeoutMs: 5 }),
