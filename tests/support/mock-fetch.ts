 ...
 * be a defect (docs/testing.md, layer table). Every test injects this fetch
 * via `new GetItDone({ fetch })` — the SDK's sanctioned test-injection seam.
 */
import { vi } from 'vitest'

import { GetItDone, type GetItDoneOptions } from '../../src/client'

export const TEST_API_KEY = 'gid_test_unit'
export const TEST_BASE_URL = 'https://api.getitdone.test'
...
export interface QueuedRejection {
    rejectWith: unknown
}
...
export function createQueuedFetch(
    ...
    const fetchFn = vi.fn(
        async (
            ...
            const bodyText =
                step.textBody ??
                (step.jsonBody !== undefined
                    ? JSON.stringify(step.jsonBody)
                    : null)
...
export function problemReply(
    ...
    return {
        ...
        jsonBody: {
            type: `https://nowgetitdone.com/docs/api/problems/${code.replaceAll('_', '-')}`,
            title: code,
            status,
            code,
...
export function makeTestClient(
    ...
    options: Partial<GetItDoneOptions> = {},
): GetItDone {
    return new GetItDone({
        apiKey: TEST_API_KEY,
